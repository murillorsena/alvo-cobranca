...
export class CollectionActionRepositoryPostgre implements CollectionActionRepository {
    ...
    async save (collectionAction: CollectionAction): Promise<void> {
        const { id, type, content, createdAt, updatedAt, storeId, userId } = collectionAction;
        const statement = `
            ...
            RETURNING *;
        `;
        const params = [ id, type, content, createdAt, updatedAt, storeId, userId ];
        await this.connection.query(statement, params);
    }
    
    async update (collectionAction: CollectionAction): Promise<void> {
        const { id, type, content, updatedAt } = collectionAction;
        const statement = `
            UPDATE
                "collection_action"
            SET
    ...
    async findAll (): Promise<CollectionAction[]> {
        const statement = `
            ...
            JOIN
                "user"
            ON
                "collection_action"."user_id" = "user"."id";
        `;
