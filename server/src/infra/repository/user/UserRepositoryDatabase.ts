...
export default class UserRepositoryDatabase implements UserRepository {
    ...
    async findAll (): Promise<User[]> {
        ...
        for (const userData of usersData) {
            users.push(User.buildExistingUser(userData.name, userData.email, userData.password, userData.id));
        }
        return users;
    }
    ...
    async findById (id: string): Promise<User | null> {
        ...
        const user = User.buildExistingUser(userData.name, userData.email, userData.password, userData.id);
        return user;
    }
